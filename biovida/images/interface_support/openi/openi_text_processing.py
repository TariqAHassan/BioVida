...
def _imaging_modality_guess_info():
    """
    ...
    modality_subtypes = {
        'ct': [
            ...
            ['spinal', 'spine'],
            ['segmentation'],
            ['non-contrast', 'non contrast', 'noncontrast', 'w/o contrast', 'unenhanced'],
            ['pre-contrast', 'precontrast'],
            ['post-contrast', 'postcontrast'],
            ['contrast-enhanced', 'contrast enhanced', 'enhanced contrast', 'w/contrast', 'contrast CT'],
        ],
        'mri': [
            [' gadolinium ', ' gad '],
            ...
            ['pre-contrast', 'precontrast'],
            ['post-contrast', 'postcontrast'],
            ['contrast-enhanced', 'contrast enhanced', 'enhanced contrast', 'w/contrast', 'contrast MR'],
            ['localizer'],
    ...
    anatomical_areas = ['abdomen', 'chest', 'brain', 'spinal']

    # Add all possible contradictions generated by 'abdomen', 'chest', 'brain' and 'spinal'.
    contradictions += list(map(list, combinations(anatomical_areas, r=2)))

    # Generate a dictionary of subtypes which are particular to a given modality (e.g., T2 to MIR).
    ...
    for k, v in modality_subtypes.items():
        new_value = [[j for j in i if unique_subtypes.count(cln(j)) == 1] for i in v if i[0] not in anatomical_areas]
        modality_specific_subtypes[k] = list(filter(None, new_value))

    return terms_dict, modality_subtypes, contradictions, {k: v for k, v in modality_specific_subtypes.items() if len(v)}


# Generate terms required for ``_imaging_modality_guess()``
_terms_dict, _modality_subtypes, _contradictions, _modality_specific_subtypes = _imaging_modality_guess_info()
...
def _imaging_modality_guess(abstract, image_caption, image_mention):
    ...
    def scan(source):
        ...
        for k, v in _terms_dict.items():
            if any(i in source for i in v[0]):
                ...
                if k in _modality_subtypes:
                    for v2 in _modality_subtypes[k]:
        ...
        for k, v in _modality_specific_subtypes.items():
            for i in v:
    ...
    def formatter(dictionary):
        """Format the output; form: 'formal name: subtype1; subtype2; ...'."""
        modality = list(dictionary.keys())[0]
        modality_formal_name = _terms_dict[modality][-1]
